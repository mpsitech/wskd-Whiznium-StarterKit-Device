...
uint8_t VecVWskdIcicleController::getTix(
			const string& sref
		) {
...
string VecVWskdIcicleController::getSref(
			const uint8_t tix
		) {
...
void VecVWskdIcicleController::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
uint8_t VecVWskdIcicleState::getTix(
			const string& sref
		) {
	string s = StrMod::lc(sref);
...
string VecVWskdIcicleState::getSref(
			const uint8_t tix
		) {
	if (tix == NC) return("nc");
...
string VecVWskdIcicleState::getTitle(
			const uint8_t tix
		) {
...
void VecVWskdIcicleState::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
uint8_t VecWWskdIcicleBuffer::getTix(
			const string& sref
		) {
	string s = StrMod::lc(sref);
	...
	else if (s == "flgbuffromfeatdet") return FLGBUFFROMFEATDET;
	else if (s == "pvwabuffromcamacq") return PVWABUFFROMCAMACQ;
	else if (s == "pvwbbuffromcamacq") return PVWBBUFFROMCAMACQ;

	return(0);
};

string VecWWskdIcicleBuffer::getSref(
			const uint8_t tix
		) {
	if (tix == CMDRETTOHOSTIF) return("cmdretToHostif");
	else if (tix == HOSTIFTOCMDINV) return("hostifToCmdinv");
	else if (tix == FLGBUFFROMFEATDET) return("flgbufFromFeatdet");
	else if (tix == PVWABUFFROMCAMACQ) return("pvwabufFromCamacq");
	else if (tix == PVWBBUFFROMCAMACQ) return("pvwbbufFromCamacq");

	return("");
};

void VecWWskdIcicleBuffer::fillFeed(
			Feed& feed
		) {
	feed.clear();

	std::set<uint8_t> items = {CMDRETTOHOSTIF,HOSTIFTOCMDINV,FLGBUFFROMFEATDET,PVWABUFFROMCAMACQ,PVWBBUFFROMCAMACQ};
